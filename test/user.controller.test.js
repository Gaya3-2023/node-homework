...
describe("testing logon, register, and logoff", () => {
  ...
  it("35. A string in the cookie array starts with 'jwt='.",() => {
    ...
    hasjwtCookie = setCookieArray.some((str) => str.startsWith("jwt="));
    expect(hasjwtCookie).toBe(true);
  });
  it("36. That string contains 'HttpOnly;'.  (This is a security test!)",() => {
    const setCookieArray = saveRes.get("Set-Cookie");
    hasHttp = setCookieArray.some((str)=> str.includes("HttpOnly"))
...
describe("Testing JWT middleware", () =>{
  ...
  it("64. Calls next() if both the token and the jwt are good.",async ()=>{
    ...
    req.headers["X-CSRF-TOKEN"] = "goodtoken";
    const next = await waitForRouteHandlerCompletion(jwtMiddleware,
      req,
      saveRes,
    );
    saveReq = req;
